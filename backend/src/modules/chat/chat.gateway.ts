  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleAuthenticate(
    ...
    try {
      ...
      client.emit(CHAT_SERVER_EVENTS.ONLINE_STATUS, {
        onlineUsers: currentOnlineUsers.reduce(
          (acc, id) => {
            acc[id] = true;
            return acc;
          },
      ...
      this.logger.debug(
        `User ${userId} authenticated with socket ${client.id}`,
      );
    ...
    try {
      ...
      for (const participant of participants) {
        ...
        this.server
          ...
          .emit(CHAT_SERVER_EVENTS.NEW_MESSAGE, {
            message,
            conversationId,
          });
      }
      ...
      if (senderId) {
        ...
        if (senderSockets) {
          ...
          this.server
            .to(roomName)
            .emit(CHAT_SERVER_EVENTS.NEW_MESSAGE, { message, conversationId });
        }
      } else {
  ...
  emitConversationUpdate(userId: string, conversation: any) {
    this.server
      .to(`user:${userId}`)
      .emit(CHAT_SERVER_EVENTS.CONVERSATION_UPDATED, {
        conversation,
      });
  }
  ...
  emitUserBlocked(blockerId: string, blockedId: string) {
    ...
    this.logger.debug(
      `User ${blockerId} blocked user ${blockedId} - notified both parties`,
    );
  }
  ...
  emitUserUnblocked(unblockerId: string, unblockedId: string) {
    ...
    this.server
      .to(`user:${unblockedId}`)
      .emit(CHAT_SERVER_EVENTS.USER_UNBLOCKED, {
    ...
    this.logger.debug(
      `User ${unblockerId} unblocked user ${unblockedId} - notified both parties`,
    );
  }
  ...
  private async notifyMessageDelivered(
    ...
    if (onlineRecipients.length > 0) {
      ...
      this.server
        .to(`user:${senderId}`)
        .emit(CHAT_SERVER_EVENTS.MESSAGE_DELIVERED, {
          conversationId,
          messageId,
