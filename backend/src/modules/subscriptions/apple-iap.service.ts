...
const APPLE_SERVER_API_SANDBOX =
  'https://api.storekit-sandbox.itunes.apple.com';
const APPLE_LEGACY_VERIFY_PRODUCTION =
  'https://buy.itunes.apple.com/verifyReceipt';
const APPLE_LEGACY_VERIFY_SANDBOX =
...
export class AppleIapService {
  ...
  async generateToken(): Promise<string> {
    ...
    if (!keyId || !issuerId || !bundleId || !pem) {
      throw new Error(
        'Missing APPLE_IAP_KEY_ID, APPLE_IAP_ISSUER_ID, APPLE_BUNDLE_ID or APPLE_IAP_PRIVATE_KEY',
      );
    }
  ...
  > {
    if (!this.hasServerApiConfig()) {
      return {
        ok: false,
        error: 'Apple IAP key not configured (APPLE_IAP_* in .env)',
      };
    }
    ...
    for (const env of ['Production', 'Sandbox'] as const) {
      const baseUrl =
        env === 'Production'
          ? APPLE_SERVER_API_PRODUCTION
          : APPLE_SERVER_API_SANDBOX;
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (env === 'Sandbox')
            return { ok: false, error: `Apple API: ${res.status} ${text}` };
          continue;
        }
        const data = (await res.json()) as { signedTransactionInfo?: string };
        const signed = data?.signedTransactionInfo;
        if (!signed)
          return { ok: false, error: 'No signedTransactionInfo in response' };

        const info = this.decodeSignedTransactionInfo(signed);
        if (!info)
          return { ok: false, error: 'Failed to decode signedTransactionInfo' };
    ...
    return {
      ok: false,
      error: 'Transaction not found in Production or Sandbox',
    };
  }

  /** Decode JWS payload (middle part) without full signature verification for simplicity. */
  private decodeSignedTransactionInfo(
    signed: string,
  ): AppleTransactionInfoDecoded | null {
    try {
  ...
  async verifyReceiptLegacy(receiptData: string): Promise<{
    ...
    const urls = [
      ...
      process.env.APPLE_VERIFY_RECEIPT_SANDBOX_URL ||
        APPLE_LEGACY_VERIFY_SANDBOX,
    ];
    ...
    for (const url of urls) {
      try {
        ...
        if (data.status === 0) {
          ...
          const origId =
            latest?.original_transaction_id ?? latest?.transaction_id;
